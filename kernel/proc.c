...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      initlock(&p->lock, "proc");//proc[NPROC]是全局变量
...
found:
  ...
  // void memu* = kalloc();
  // p->trapframe  = (sttuct trapframe*)memu;
  //if(p->trapframe == 0);
  ...
  p->pagetable = proc_pagetable(p);//create a new user pagetable
  if(p->pagetable == 0){
    freeproc(p);
    release(&p->lock);
    return 0;
  }
  // 为新进程创建独立的内核页表，并将内核所需要的各种映射添加到新的页表
  ...
  if (pa == 0)
    panic("kallo");
  uint64 va = KSTACK((int)0);//que:     // 将内核栈映射到固定的逻辑地址上    
...
kama_kvmcopymappings(np->pagetable, np->kama_kernelpgtbl, 0, p->sz) < 0) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        //执行 swtch 前：CPU 运行的是调度器；
        //执行 swtch 后：CPU 切换到目标进程运行
